...
mod tests {
	...
	/// across many (ν, ν', K, R) configurations.
	#[test]
	...
	#[test]
	fn alpha_value_independent_of_mask_entries() {
		let nu = 3u32;
		let nu_prime = 5u32;
		let m = 1usize << nu;
		let n = 1usize << nu_prime;
		...
		let mut c_a = vec![g(0); n];
		let mut c_b = vec![g(0); n];
		for k in 0..m {
			c_a[k] = g(31 + k as u64);
			c_b[k] = g(31 + k as u64);
		}
		for k in m..n {
			c_a[k] = g(1_000 + k as u64);
			c_b[k] = g(9_999 + k as u64);
		}
		let dot_a: Goldilocks4 = c_a.iter().zip(alpha_vec.iter()).map(|(c, a)| *c * *a).sum();
		let dot_b: Goldilocks4 = c_b.iter().zip(alpha_vec.iter()).map(|(c, a)| *c * *a).sum();
