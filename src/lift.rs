...
//! ```
//!
//! With LSB-first storage the multilinear extension factors as
//!
//! ```text
//! u(x)(s₁, …, s_ν)  =  ∏_{j=1}^{ν}  F_j(s_j)        where    F_j(s) = 1 − s + s · x^{2^{j-1}}.
//! ```
//!
//! This module exposes [`MonomialLift`], which stores the precomputed
//! `(x, x², x⁴, …, x^{2^{ν-1}})` powers and can compute either the explicit
//! length-`M` vector ([`MonomialLift::materialize`]) or its sumcheck-folded
//! form ([`MonomialLift::folded`]). The verifier uses the latter (via
//! [`crate::alpha::BatchedAlpha`]) to avoid materialising any length-`M`
//! vector on the verifier side.

use crate::field::Goldilocks4;

/// A single monomial-basis lift `u(x)` parameterised by `ν` and the
/// evaluation point `x`.
...
mod tests {
	...
	fn folded_full_returns_single_value() {
		let lift = MonomialLift::new(g(5), 4);
