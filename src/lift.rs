...
//! ```
//!
//! padded with `N − M` zeros at the mask region. With LSB-first storage the
//! multilinear extension factors as
//!
//! ```text
//! u(x)(s₁, …, s_{ν'})  =  ∏_{j=1}^{ν}  F_j(s_j)  ·  ∏_{j=ν+1}^{ν'} (1 − s_j)
//!                where    F_j(s) = 1 − s + s · x^{2^{j-1}}.
//! ```
//!
//! This module exposes [`MonomialLift`], which stores the precomputed
//! `(x, x², x⁴, …, x^{2^{ν-1}})` powers and can compute either the explicit
//! length-`N` vector ([`MonomialLift::materialize`]) or its
//! sumcheck-folded form ([`MonomialLift::folded`]). The verifier uses the
//! latter (via [`crate::alpha::BatchedAlpha`]) to avoid materialising any
//! length-`N` vector on the verifier side.
...
impl MonomialLift {
	...
	/// `(1, x, x², …, x^{M-1}, 0, 0, …, 0)`.
	///
	/// The prover uses this to materialise `α` for the WHIR `LinearForm`
	/// input. The verifier never calls it.
...
mod tests {
	...
	#[test]
	fn materialize_zero_on_masks() {
		let lift = MonomialLift::new(g(7), 3, 5);
		let v = lift.materialize();
		assert_eq!(v.len(), 32);
		for (k, val) in v.iter().enumerate() {
			if k >= 8 {
				assert!(val.is_zero(), "mask position {k} should be zero");
			}
		}
	}

	#[test]
	fn materialize_matches_horner_for_k_lt_m() {
		let x = g(7);
